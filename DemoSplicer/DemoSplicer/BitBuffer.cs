...
namespace DemoSplicer
{
	...
	public class BitBuffer
	{
		...
		public bool SeekBits(int count)
		{
			return SeekBits(count, SeekOrigin.Current);
		}

		public bool SeekBits(int offset, SeekOrigin origin)
		{
			switch (origin)
			{
		...
		private uint ReadUnsignedBitsLittleEndian(int nBits)
		{
			...
			if (nBits <= 0 || nBits > 32)
			{
				throw new ArgumentException(@"Value must be a positive integer between 1 and 32 inclusive. Was " + nBits, nameof(nBits));
			}
